package cn.peyton.plum.mall.service.impl;

import cn.peyton.plum.core.inf.BaseConvertBo;
import cn.peyton.plum.core.inf.mapper.IBaseMapper;
import cn.peyton.plum.core.inf.service.AbstractAppRealizeService;
import cn.peyton.plum.mall.bo.RolesMenusBo;
import cn.peyton.plum.mall.mapper.RolesMenusMapper;
import cn.peyton.plum.mall.param.RolesMenusParam;
import cn.peyton.plum.mall.pojo.RolesMenus;
import cn.peyton.plum.mall.service.RolesMenusService;
import jakarta.annotation.Resource;
import org.springframework.stereotype.Service;

/**
 * <h3> 角色菜单关联 Service 实现类</h3>
 * <pre>
 * @author <a href="http://www.peyton.cn">peyton</a>
 * @mail <a href="mailto:fz2919@tom.com">fz2919@tom.com</a>
 * @date 2023年11月11日 18:10:52
 * @version 1.0.0
 * </pre>
 */
@Service("rolesMenusService")
public class RolesMenusServiceImpl extends AbstractAppRealizeService<Long, RolesMenus, RolesMenusParam> implements RolesMenusService {
    @Resource
    private RolesMenusMapper rolesMenusMapper;

    @Override
    public BaseConvertBo<RolesMenus, RolesMenusParam> initBo() {
        return new RolesMenusBo();
    }

    @Override
    public IBaseMapper<Long, RolesMenus> initMapper() {
        return rolesMenusMapper;
    }

}
