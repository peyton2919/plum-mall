 ...
 * </pre>
*/
public interface RoleUserMapper extends IBaseMapper<Long, RoleUser> {
     ...
     */
    int batchAdd(@Param("userRoleList") List<RoleUser> roleUserList);
     ...
     * @param record 用户角色对象
     */
    RoleUser selectByThree(RoleUser record);

    /**
     * <h4>是否是超级管理员</h4>
     * @param shareId 用户Id
     * @param shareType 用户类型
     * @return 受影响行数 > 0 表示为超级管理员
     */
    int isSuperAdmin(Long shareId, int shareType);
}
