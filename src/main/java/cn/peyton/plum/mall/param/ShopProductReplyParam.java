...
public class ShopProductReplyParam implements Serializable {
	 ...
	 * <pre>
	 * 	 转换字段如下:
	 * 	 [id,memberId,oid,uniqueCode,productId,replyType,productScore,serviceScore,comment,pics,createTime,merchantReplyContent,merchantReplyTime,isDel,isReply]
	 * </pre>
	 */
	public ShopProductReply convert(){
		ShopProductReply shopProductReply = new ShopProductReply(); 
		shopProductReply.setId(id);
		shopProductReply.setMemberId(memberId);
		...
		shopProductReply.setComment(comment);
		shopProductReply.setPics(pics);
	...
	public ShopProductReplyParam compat(ShopProductReply shopProductReply){ 
		...
		this.setMemberId(shopProductReply.getMemberId());
		...
		this.setComment(shopProductReply.getComment());
		this.setPics(shopProductReply.getPics());
