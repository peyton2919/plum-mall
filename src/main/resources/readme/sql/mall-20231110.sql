...
(
	...
	`share_type` tinyint(4) not null default 2 comment '用户类型 ,默认 2 员工、 0 会员、1 供应商',
    `role_id` bigint(20) NOT NULL COMMENT '角色ID',
    PRIMARY KEY (`id`)
...
(
    ...
    `level`       tinyint(4)    NOT NULL DEFAULT '1' COMMENT '等级：1一级佣金，2二级佣金',
    `order_id`    int(11)       NULL COMMENT '订单ID',
    `commission`  decimal(8, 2) NOT NULL DEFAULT '0.00' COMMENT '佣金',
    `status`      tinyint(4)    NOT NULL DEFAULT '0' COMMENT '状态 0冻结中，1生效，-1失效',
    `explain`     varchar(250)  NULL COMMENT '说明',
